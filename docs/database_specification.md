...
* **`task_user_progresses` テーブル（更新後）**| task_id (FK) | user_id (FK)  | user_name | is_completed          |
  | :----------- | :------------ | :-------- | :-------------------- |
  | `tsk-go`   | `usr-alice` | Alice     | **false**       |
...
## 7. 構造体タグによるマッピング（JSONタグとORMタグの設計）

Go言語におけるデータ構造（構造体）は、Web API（JSON通信）やデータベース永続化（GORM）など、異なるレイヤーのインターフェースとやり取りする際に、**「構造体タグ（Struct Tag）」**と呼ばれる注釈を利用してマッピング方法を指定している。

### ① JSONタグ (`json:"..."`)

APIのインプット・アウトプットとして使用される際のJSONキー名を規定する。

* **役割**: Go言語の規約上、外部パッケージに公開するフィールド名は必ず大文字始まり（例: `ID`, `Name`）となる。しかし、Webクライアント（JavaScript）との通信では小文字（`id`, `name`）が一般的に好まれる。JSONタグを付与することで、この表記の差異を自動でマッピングする。
* **定義例**:
  ```go
  ID string `json:"id"`
  ```

  この場合、Goコード内では大文字の `.ID` でアクセスするが、フロントエンドに返却されるJSONやフロントエンドから送信されてくるJSONでは、すべて小文字の `"id"` というキー名が割り当てられる。

### ② ORMタグ（GORMタグ） (`gorm:"..."`)

GoのORMライブラリである GORM に対して、データベースのテーブルスキーマ制約やアソシエーション（関連）の挙動を指示する。

* **主要なパラメータとその意味**:
  * `gorm:"primaryKey"`: このカラムを主キー（PK）として指定する。データベース内での値の一意性が保証され、検索用インデックスが自動生成される。
  * `gorm:"unique"`: このカラムの値の重複を禁止する（ユニーク制約）。メールアドレス（`email`）や招待コード（`invite_code`）などに適用される。
  * `gorm:"many2many:中間テーブル名"`: 多対多の関係を定義する（例: `User` と `Group` の関係）。GORMが裏側で自動的に中間テーブル（`user_groups`）を介してデータを結合・更新する。
  * `gorm:"foreignKey:外部キー名"`: 1対多の関係（例: `Task` と `TaskUserProgress`）において、親オブジェクトと紐づく子の外部キーカラム名を指定する。
* **定義例**:
  ```go
  Email string `json:"email" gorm:"unique"`
  ```

  この場合、API通信上は `"email"` というキー名になり、PostgreSQL上のテーブルでは `email` 列として保存され、かつ値が重複しないようにユニーク制約が付与される。

### ③ アーキテクチャ設計におけるトレードオフ（ドメイン層への記述）

ドメインモデル（[domain/user.go](file:///home/yuma25/github/Uni-Steps/backend/domain/user.go) など）に直接 `json` や `gorm` タグを記述する現在の設計には、以下のようなメリットとデメリットが存在する。

#### メリット（実利優先）

* **開発効率の最大化**: 1つの構造体で「DBスキーマ」「ビジネスロジック」「API I/O仕様」をすべて表現できるため、モデル間のデータ詰め替え（マッピング）処理を書く必要がなく、コード量が劇的に削減される。

#### デメリット（アーキテクチャの妥協点）

* **密結合（密な依存）**: ドメイン層が、データベース構造（GORM）やWebの通信仕様（JSON）に依存してしまう。本来、クリーンアーキテクチャの思想ではドメイン層は「技術的な都合から完全に分離された純粋な状態」であるべきとされる。

#### 代替アプローチ（マルチモデルパターン）

もし将来的にアプリケーションの規模が大きくなり、APIのデータ構造とDBの物理構造に大きな乖離が発生した場合は、ドメインモデル（タグなし）から、API用のDTO（JSONタグ付き）とDB用エンティティ（GORMタグ付き）を完全に分離させ、マッパー関数を介してデータをやり取りする設計へと移行する。
