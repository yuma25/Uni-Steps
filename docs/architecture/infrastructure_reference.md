...
*   🔧 **メソッド: `Save(ctx, task) error`**
    ...
    *   **こだわり**: トランザクションを使用し，`UserProgress` は一度全削除してから再登録する方式を採用している．保存直前に親 `Task` の ID を各 `UserProgress` レコードに明示的にセットすることで，外部キー制約違反（SQLSTATE 23503）を確実に防止している．
...
---
*最終更新日: 2026年6月15日*
: フロントエンド側はトークン消滅を検知すると，ブラウザの権限がある場合に限り「サイレント再登録（handleSilentResubscribe）」を試みるため，通知機能は自動的に復旧する．
    *   **引数**: `ctx`, `userID` (`string`), `message` (`string`), `targetURL` (`string`)．
    *   **戻り値**: 成功時は nil，失敗時はエラー．
